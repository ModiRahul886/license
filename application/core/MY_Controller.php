...
class Admin_Controller extends Common_Libraries
{
	function __construct()
	{
		...
		$this->data['admindetail'] = $this->common_model->get_admin_details($this->data['license_admin_info']['iAdminId'], '');

		$this->data['hotel_url'] = $this->config->item('hotel_url');
		/* Admin Module Permission*/
		/* Get Module*/
		if ($this->data['license_admin_info']['eRole'] == 'Subadmin') {
			...
			if ($this->data['license_admin_info']['eRole'] == 'Subadmin') {
				$admindetail = $this->common_model->get_adminper_details($this->data['license_admin_info']['iAdminId'], 'Subadmin');
...
class Hotel_Controller extends Common_Libraries
{
	function __construct()
	{
		parent::__construct();
		...
		$this->data['base_image'] = $this->config->item('base_image');
		$this->data['admin_boostrap'] = $this->config->item('admin_boostrap');
		$this->data['happyhour_hotel_info'] = $this->session->userdata['happyhour_hotel_info'];
		$this->data['admin_base_url'] = $this->config->item('base_url');
		$this->data['hoteldetail'] = $this->common_model->get_hotel_details($this->data['happyhour_hotel_info']['iOwnerId'], '');
		$this->data['hotel_url'] = $this->config->item('hotel_url');
		/* Admin Module Permission*/
		/* Get Module*/
		if ($this->data['happyhour_hotel_info']['eRole'] == 'Subadmin') {
			$leftsidebar = $this->common_model->getleftbar();
			foreach ($leftsidebar as $key => $value) {
				$leftsidebar[$key]['submodule'] = $this->getsubmodule($value['iModuleId']);
			}
			...
			if ($this->data['happyhour_hotel_info']['eRole'] == 'Subadmin') {
				$hoteldetail = $this->common_model->get_adminper_details($this->data['happyhour_hotel_info']['iOwnerId'], 'Subadmin');

				$hoteldetail['tListing'] = explode(",", $hoteldetail['tListing']);
				$hoteldetail['tAdd'] = explode(",", $hoteldetail['tAdd']);
				$hoteldetail['tUpdate'] = explode(",", $hoteldetail['tUpdate']);
				$hoteldetail['tDelete'] = explode(",", $hoteldetail['tDelete']);
				$hoteldetail['tActive'] = explode(",", $hoteldetail['tActive']);
				$hoteldetail['tInactive'] = explode(",", $hoteldetail['tInactive']);

				$hoteldetail['tExportMerchant'] = explode(",", $hoteldetail['tExportMerchant']);
				$hoteldetail['tExportUsers'] = explode(",", $hoteldetail['tExportUsers']);
				$hoteldetail['tSendPushnotification'] = explode(",", $hoteldetail['tSendPushnotification']);

				for ($i = 0; $i < count($hoteldetail['tListing']); $i++) {
					$list[$i] = $this->common_model->getparentid($hoteldetail['tListing'][$i]);
				}
				if ($hoteldetail['iAccessId'] != '') {
					$hoteldetail['tListing'] = array_unique(array_merge($list, $hoteldetail['tListing']));
				}
				$this->data['adminpermidetail'] = $hoteldetail;
			} else {
				$hoteldetail = $this->common_model->get_adminper_details($this->data['happyhour_hotel_info']['iOwnerId'], 'Subadmin');
			}
